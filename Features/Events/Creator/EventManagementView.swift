...
struct EventManagementView: View {
    ...
    @State private var selectedGuestProfile: GuestRecord? = nil
    ...
    private var expectedAttendanceCount: Int {
        acceptedCRMCount + checkedInCRMCount
    }

    private var noShowCount: Int {
        max(0, acceptedCRMCount)
    }

    private var attendanceRatePercent: Int {
        guard expectedAttendanceCount > 0 else { return 0 }
        return min(100, Int((Double(checkedInCRMCount) / Double(expectedAttendanceCount)) * 100.0))
    }

    private var rsvpResponseRatePercent: Int {
        let totalKnown = invitedCRMCount + acceptedCRMCount + declinedCRMCount + checkedInCRMCount
        let responded = acceptedCRMCount + declinedCRMCount + checkedInCRMCount
        guard totalKnown > 0 else { return 0 }
        return min(100, Int((Double(responded) / Double(totalKnown)) * 100.0))
    }

    private var noShowRatePercent: Int {
        guard expectedAttendanceCount > 0 else { return 0 }
        return max(0, 100 - attendanceRatePercent)
    }
    ...
    private func commandSheetHost(_ command: EventCommandSheet) -> AnyView {
        AnyView(
            commandSheet(command)
                ...
                .sheet(item: $selectedGuestProfile) { guest in
                    attendeeProfileSheet(guest)
                        .presentationDetents([.medium, .large])
                        .presentationDragIndicator(.visible)
                        .presentationCornerRadius(28)
                }
    ...
    private var attendanceAnalyticsPanel: some View {
        sectionPanel(title: "ATTENDANCE ANALYTICS", subtitle: "Show-up rate, response health, and no-show signal") {
            VStack(spacing: 12) {
                HStack(spacing: 10) {
                    attendanceInsightCard(
                        title: "ATTENDANCE",
                        value: "\(attendanceRatePercent)%",
                        subtitle: "\(checkedInCRMCount)/\(max(1, expectedAttendanceCount)) checked in",
                    ...
                    attendanceInsightCard(
                        title: "RSVP RATE",
                        value: "\(rsvpResponseRatePercent)%",
                        subtitle: "responses captured",
                ...
                HStack(spacing: 10) {
                    attendanceInsightCard(
                        title: "NO-SHOW",
                        value: "\(noShowRatePercent)%",
                        subtitle: "\(noShowCount) accepted pending",
                        icon: "person.crop.circle.badge.exclamationmark",
                        tint: noShowCount > 0 ? .orange : .green
                    )

                    attendanceInsightCard(
                        title: "CAPACITY",
                        value: currentEvent.capacity > 0 ? "\(liveCapacityPercent)%" : "OPEN",
                        subtitle: currentEvent.capacity > 0 ? "\(liveCRMCapacityCount)/\(currentEvent.capacity) filled" : "\(liveCRMCapacityCount) confirmed",
                        icon: "gauge.with.dots.needle.67percent",
                        tint: liveCapacityPercent >= 90 ? .orange : .green
                    )
                }
            }
        }
    ...
    private func attendanceInsightCard(
        title: String,
        value: String,
        subtitle: String,
        icon: String,
        ...
        .padding(12)
        .background(RoundedRectangle(cornerRadius: 17, style: .continuous).fill(tint.opacity(0.075)))
        .overlay(RoundedRectangle(cornerRadius: 17, style: .continuous).stroke(tint.opacity(0.16), lineWidth: 1))
    }
    ...
    private func guestRow(_ guest: GuestRecord) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                HStack(spacing: 8) {
                    Button {
                        selectedGuestProfile = guest
                    } label: {
                        guestRowActionLabel("PROFILE", "person.crop.circle.fill", tint: .orange)
                    }
                    .buttonStyle(.plain)
    ...
    private func guestCheckInPassStrip(_ guest: GuestRecord) -> some View {
        Button {
            copyGuestCheckInPass(guest)
        } label: {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(guestCheckInPassCode(guest))
                        .font(.system(size: 9, weight: .black, design: .monospaced))
                        .foregroundColor(.white.opacity(0.42))
                        .tracking(0.5)
                        .lineLimit(1)
                ...
                Text(guest.status == .checkedIn ? "USED" : "COPY")
                    .font(.system(size: 8, weight: .black, design: .monospaced))
                    .foregroundColor(guest.status == .checkedIn ? .black.opacity(0.86) : .orange.opacity(0.92))
                    .padding(.horizontal, 8)
                    .frame(height: 22)
                    .background(Capsule().fill(guest.status == .checkedIn ? Color.green.opacity(0.92) : Color.orange.opacity(0.10)))
            }
            .padding(10)
            .background(RoundedRectangle(cornerRadius: 15, style: .continuous).fill(Color.white.opacity(0.045)))
            .overlay(RoundedRectangle(cornerRadius: 15, style: .continuous).stroke(Color.orange.opacity(0.12), lineWidth: 1))
        }
        .buttonStyle(.plain)
    }

    private func guestCheckInPassCode(_ guest: GuestRecord) -> String {
        let eventSeed = currentEvent.id.prefix(6).uppercased()
        let guestSeed = stableCRMGuestDocumentID(for: guest).suffix(8).uppercased()
        return "TG-\(eventSeed)-\(guestSeed)"
    }

    private func copyGuestCheckInPass(_ guest: GuestRecord) {
        let passCode = guestCheckInPassCode(guest)
        let payload = """
        Trivia GOAT Check-In Pass
        ...
        app.showEventToast("PASS COPIED")
    }
    
    private func attendeeProfileSheet(_ guest: GuestRecord) -> some View {
        ZStack {
            Color.black.ignoresSafeArea()

            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 16) {
                    attendeeProfileHero(guest)
                    attendeeProfileCommunications(guest)
                    attendeeProfileAccess(guest)
                    attendeeProfileSessions(guest)
                    attendeeProfileActions(guest)
                }
                .padding(20)
                .padding(.bottom, 24)
            }
    ...
    private func attendeeProfileHero(_ guest: GuestRecord) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                speakerOrGuestAvatar(guest)
    ...
    private func attendeeProfileCommunications(_ guest: GuestRecord) -> some View {
        sectionPanel(title: "COMMUNICATIONS", subtitle: "Attendee-specific CRM activity") {
            VStack(alignment: .leading, spacing: 10) {
                Text("Direct messaging active")
                Text("Communication history will attach to event logs next pass")
            }
            .font(.system(size: 12, weight: .bold, design: .rounded))
    ...
    private func attendeeProfileAccess(_ guest: GuestRecord) -> some View {
        sectionPanel(title: "EVENT ACCESS", subtitle: "Check-in pass and arrival state") {
            VStack(spacing: 10) {
                guestCheckInPassStrip(guest)
    ...
    private func attendeeProfileSessions(_ guest: GuestRecord) -> some View {
        let assigned = sessionRecords.filter { $0.speakerIDs.contains(stableCRMGuestDocumentID(for: guest)) }

        return sectionPanel(title: "SESSIONS", subtitle: guest.role == .speaker ? "Speaker itinerary" : "Assigned itinerary") {
            if assigned.isEmpty {
                Text(guest.role == .speaker ? "No speaker sessions assigned yet." : "No assigned sessions yet.")
                    .font(.system(size: 12, weight: .bold, design: .rounded))
                    .foregroundColor(.white.opacity(0.48))
            } else {
                VStack(spacing: 8) {
    ...
    private func attendeeProfileActions(_ guest: GuestRecord) -> some View {
        VStack(spacing: 10) {
            Button {
                selectedGuestProfile = nil
                openIndividualCommunication(for: guest)
            } label: {
                commandActionLabel("MESSAGE ATTENDEE", "envelope.fill", filled: true)
            }
            .buttonStyle(.plain)
            ...
            Button {
                ...
                commandActionLabel("EDIT PROFILE", "square.and.pencil", filled: false)
            }
            .buttonStyle(.plain)
            ...
            Button {
                ...
                commandActionLabel("CHECK IN", "checkmark.seal.fill", filled: false)
            }
            .buttonStyle(.plain)
            .disabled(guest.status == .checkedIn || guest.status == .declined)
            .opacity((guest.status == .checkedIn || guest.status == .declined) ? 0.55 : 1)
    ...
    private func commandSheet(_ command: EventCommandSheet) -> AnyView {
        ...
        switch command {
        case .invitationCenter, .guestPanel, .delegateDirectory:
            bodyContent = AnyView(guestDirectoryPanel(command))

        case .staffRoles:
            bodyContent = AnyView(staffRolesPanel)
    ...
    private func commandSummaryCard(_ command: EventCommandSheet) -> some View {
        let metrics = commandSummaryMetrics(for: command)
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                ForEach(Array(metrics.enumerated()), id: \.offset) { _, metric in
                    commandMetric(metric.title, metric.value)
                }
            }
        }
        .padding(16)
    ...
    private func commandSummaryMetrics(
        for command: EventCommandSheet
    ) -> [(title: String, value: String)] {
        let records = directoryGuestRecords
        let accepted = records.filter { $0.status == .accepted || $0.status == .checkedIn }.count
        let checkedIn = records.filter { $0.status == .checkedIn }.count
        let speakers = records.filter { $0.role == .speaker || $0.role == .moderator }.count
        let staffReady = records.filter {
            $0.role == .staff || $0.role == .moderator || $0.role == .vip || $0.isVIP
        }.count

        switch command {
        case .invitationCenter:
            return [
                ("STAGED", "\(stagedInviteCount)"),
                ("INVITED", "\(invitedCRMCount)"),
                ("ACCEPTED", "\(accepted)")
            ]

        case .delegateDirectory, .guestPanel:
            return [
                ("PEOPLE", "\(records.count)"),
                ("ACCEPTED", "\(accepted)"),
                ("CHECK-IN", "\(checkedIn)")
            ]
        ...
        default:
            return [
                ("RSVP", "\(currentEvent.attendeeCount)"),
                ("LEFT", currentEvent.capacity > 0 ? "\(max(0, currentEvent.capacity - currentEvent.attendeeCount))" : "OPEN"),
                ("WAIT", "\(currentEvent.waitlistCount)")
            ]
        }
    }
    ...
    private var staffRolesPanel: some View {
        VStack(alignment: .leading, spacing: 16) {
            staffRolesOverview
            staffRoleAssignmentsPanel
            staffAvailablePeoplePanel
        }
    }

    private var staffRolesOverview: some View {
        let staffReady = directoryGuestRecords.filter {
            $0.role == .staff || $0.role == .moderator || $0.role == .vip || $0.isVIP
        }.count

        let moderators = directoryGuestRecords.filter { $0.role == .moderator }.count
        let vipHosts = directoryGuestRecords.filter { $0.role == .vip || $0.isVIP }.count
        let speakerLiaisons = directoryGuestRecords.filter { $0.role == .speaker || $0.role == .moderator }.count

        return VStack(spacing: 10) {
            HStack(spacing: 10) {
                commandMetric("STAFF READY", "\(staffReady)")
                commandMetric("MODERATORS", "\(moderators)")
                commandMetric("VIP HOSTS", "\(vipHosts)")
            }

            HStack(spacing: 10) {
                commandMetric("SPEAKER LIAISON", "\(speakerLiaisons)")
                commandMetric("CHECKED IN", "\(checkedInCRMCount)")
                commandMetric("PEOPLE", "\(directoryGuestRecords.count)")
            }
        }
    }

    private var staffRoleAssignmentsPanel: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 10) {
                staffRoleRow(title: "EVENT OWNER", icon: "crown.fill", assigned: eventOwnerName)
                staffRoleRow(title: "OPERATIONS LEAD", icon: "slider.horizontal.3", assigned: firstStaffName(for: [.staff, .moderator]))
                staffRoleRow(title: "REGISTRATION LEAD", icon: "qrcode.viewfinder", assigned: firstCheckedInStaffName)
                staffRoleRow(title: "VIP HOST", icon: "star.circle.fill", assigned: firstVIPName)
                staffRoleRow(title: "SPEAKER LIAISON", icon: "mic.fill", assigned: firstSpeakerLiaisonName)
                staffRoleRow(title: "STAGE MANAGER", icon: "rectangle.3.group.fill", assigned: firstStaffName(for: [.moderator, .staff]))
            }
        }
        .padding(16)
        .background(RoundedRectangle(cornerRadius: 22, style: .continuous).fill(Color.white.opacity(0.045)))
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(assigned ?? "Unassigned")
                    .font(.system(size: 12, weight: .bold, design: .rounded))
                    .foregroundColor(assigned == nil ? .white.opacity(0.38) : .white.opacity(0.72))
                    .lineLimit(1)
            }
            ...
            Image(systemName: assigned == nil ? "circle" : "checkmark.circle.fill")
                .font(.system(size: 14, weight: .black))
                .foregroundColor(assigned == nil ? .white.opacity(0.24) : .green.opacity(0.92))
        }
        .padding(12)
        .background(RoundedRectangle(cornerRadius: 16, style: .continuous).fill(Color.black.opacity(0.34)))
    }

    private var staffAvailablePeoplePanel: some View {
        let candidates = directoryGuestRecords.filter {
            $0.role == .staff
            || $0.role == .moderator
            || $0.role == .speaker
            || $0.role == .vip
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            if candidates.isEmpty {
                Text("No staff-ready people yet. Promote delegates to staff, moderator, VIP, or speaker roles from the directory.")
                    .font(.system(size: 12, weight: .bold, design: .rounded))
                    .foregroundColor(.white.opacity(0.46))
                    .fixedSize(horizontal: false, vertical: true)
                    .padding(.vertical, 12)
    ...
    private func staffCandidateRow(_ guest: GuestRecord) -> some View {
        HStack(spacing: 12) {
            speakerOrGuestAvatar(guest)
            ...
            Button {
                ...
                Text("PROFILE")
                    ...
                    .overlay(Capsule().stroke(Color.orange.opacity(0.20), lineWidth: 1))
            }
            .buttonStyle(.plain)
        }
        .padding(12)
    ...
    private var eventOwnerName: String? {
        let organizer = currentEvent.organizerName?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        return directoryGuestRecords.first?.name
    }

    private func firstStaffName(for roles: [GuestRole]) -> String? {
        directoryGuestRecords.first { guest in
            roles.contains(guest.role)
        }?.name
    }

    private var firstCheckedInStaffName: String? {
        directoryGuestRecords.first { guest in
            guest.status == .checkedIn && (guest.role == .staff || guest.role == .moderator)
        }?.name
    }

    private var firstVIPName: String? {
        directoryGuestRecords.first { guest in
            guest.role == .vip || guest.isVIP
        }?.name
    }

    private var firstSpeakerLiaisonName: String? {
        directoryGuestRecords.first { guest in
            guest.role == .speaker || guest.role == .moderator
        }?.name
    }

    private func guestDirectoryPanel(_ command: EventCommandSheet) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            guestQuickStats(for: command)

            if command == .invitationCenter || command == .guestPanel {
                invitePipelinePanel
                contactsImportPanel
    ...
    private func guestQuickStats(for command: EventCommandSheet) -> some View {
        let records = directoryGuestRecords
        let accepted = records.filter { $0.status == .accepted || $0.status == .checkedIn }.count
        let checkedIn = records.filter { $0.status == .checkedIn }.count
        let speakers = records.filter { $0.role == .speaker || $0.role == .moderator }.count
        let vip = records.filter { $0.isVIP || $0.role == .vip }.count

        return VStack(spacing: 10) {
            HStack(spacing: 10) {
                commandMetric("PEOPLE", "\(records.count)")
                commandMetric("ACCEPTED", "\(accepted)")
                commandMetric("CHECK-IN", "\(checkedIn)")
            }

            HStack(spacing: 10) {
                commandMetric("SPEAKERS", "\(speakers)")
                commandMetric("VIP", "\(vip)")

                if command == .invitationCenter || command == .guestPanel {
                    commandMetric("STAGED", "\(stagedInviteCount)")
                } else {
                    commandMetric("READY", "\(accepted)")
                }
            }
        }
    }
...
private func sessionSpeakerPicker(selectedIDs: Binding<Set<String>>) -> some View {
    ...
    private func commandSubtitle(_ command: EventCommandSheet) -> String {
        ...
        case .delegateDirectory:
            return "View delegates, manage profiles, check-in attendees, message guests, and export directory data."
        case .guestPanel:
