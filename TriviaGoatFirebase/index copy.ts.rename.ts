...
async function generateAiReply(authorName: string, content: string): Promise<AiReplyMeta> {
  const model = createGemini(GEMINI_API_KEY.value(), getDeepModel());
...
async function moderateCommunityComment(
  ...
  try {
    const temp = JSON.parse(raw) as unknown;
    if (isRecord(temp)) {
      parsed = {
          ...
          const claimed = await db.runTransaction(async (tx: Transaction) => {
            ...
            tx.set(
              usernameRef,
              {
                ...
                displayName: display,
                displayNameKey: key,
                createdAt: usernameSnap.exists ? undefined : FieldValue.serverTimestamp(),
                updatedAt: FieldValue.serverTimestamp(),
              },
              { merge: true }
            );
...
export const submitPost = onCall(
  async (request: CallableRequest): Promise<SubmitPostResult> => {
    ...
    const data = (request.data ?? {}) as Record<string, unknown>;
    const content = asTrimmedString(data.content).slice(0, 5000);
...
export const submitComment = onCall(
  async (request: CallableRequest): Promise<SubmitCommentResult> => {
    ...
    const content = asTrimmedString(data.content).slice(0, 2000);
    const authorName = asTrimmedString(data.authorName) || "Player";
 ...
 * @return {Promise<void>} Completion promise.
 */
export const moderateCommunityCommentOnCreate = onDocumentCreated(
  {
  ...
  async (event: FirestoreEvent<DocumentSnapshot | undefined>): Promise<void> => {
    ...
    try {
      ...
      logger.error("moderateCommunityCommentOnCreate failed", { error: String(error) });
      await commentRef.set(
        {
          status: "rejected",
          moderatedAt: FieldValue.serverTimestamp(),
...
export const submitEventAccessLead = onCall(
  ...
  async (request: CallableRequest): Promise<object> => {
    ...
    const displayName = asTrimmedString(data.displayName).slice(0, 120);
    const note = asTrimmedString(data.note).slice(0, 240);
...
async function processVerifiedLead(eventId: string, email: string): Promise<string> {
  ...
  return db.runTransaction(async (tx: Transaction) => {
    ...
    if (approvedCount < capacity) {
      tx.set(
        eventRef.collection("invites").doc(syntheticUid),
        {
          uid: syntheticUid,
          email,
          displayName,
    ...
    tx.set(
      eventRef.collection("waitlist").doc(syntheticUid),
      {
        uid: syntheticUid,
        email,
        displayName,
...
export const verifyEventAccessLead = onCall(
  async (request: CallableRequest): Promise<object> => {
    ...
    if (!leadData.verificationTokenHash || !leadData.verificationExpiresAt) {
      throw new HttpsError("failed-precondition", "Verification is not available.");
    }

    const nowIso = new Date().toISOString();
    if (leadData.verificationExpiresAt < nowIso) {
      throw new HttpsError("deadline-exceeded", "Verification link expired.");
    }
